...
public class QRBill {
	...
	public static byte[] generate(Bill bill) {
		return generate(bill, QRBillOptions.builder().build());
	}
	 ...
	 *
	 * @param bill    the bill data
	 * @param options options for configuring the generation
	 * @return the generated QR bill (as a byte array encoded in the specified graphics format)
	 * @throws QRBillValidationError thrown if the bill data does not validate
	 * @see #draw
	 */
	public static byte[] generate(Bill bill, QRBillOptions options) {
		try (Canvas canvas = createCanvas(bill.getFormat())) {
			validateAndGenerate(bill, canvas, options);
	...
	public static void draw(Bill bill, Canvas canvas) {
		draw(bill, canvas, QRBillOptions.builder().build());
	}
	 ...
	 * @throws QRBillValidationError thrown if the bill data does not validate
	 */
	public static void draw(Bill bill, Canvas canvas, QRBillOptions options) {
		try {
			validateAndGenerate(bill, canvas, options);
		} catch (IOException e) {
			throw new QRBillGenerationException(e);
	...
	public static void drawSeparators(SeparatorType separatorType, boolean withHorizontalLine, Canvas canvas) {
		drawSeparators(separatorType, withHorizontalLine, canvas, QRBillOptions.builder().build());
	}
	 ...
	 * @param options            options for configuring the generation
	 */
	public static void drawSeparators(SeparatorType separatorType, boolean withHorizontalLine, Canvas canvas, QRBillOptions options) {
		...
		BillLayout layout = new BillLayout(bill, canvas, options);
		try {
			layout.drawBorder();
		} catch (IOException e) {
			throw new QRBillGenerationException(e);
	...
	private static void validateAndGenerate(Bill bill, Canvas canvas, QRBillOptions options) throws IOException {
		...
		if (bill.getFormat().getOutputSize() == OutputSize.QR_CODE_ONLY) {
			...
			BillLayout layout = new BillLayout(cleanedBill, canvas, options);
			layout.draw();
		}
	}
