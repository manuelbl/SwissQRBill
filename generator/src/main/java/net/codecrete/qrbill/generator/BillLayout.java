...
class BillLayout {
	...
	private final QRBillOptions options;
	...
	BillLayout(Bill bill, Canvas graphics, QRBillOptions options) {
		this.bill = bill;
	...
	private String formatAddressForDisplay(Address address, boolean withCountryCode) {
		StringBuilder sb = new StringBuilder();

		if (this.options.hasAddressNameDisplayLineBreakOn() && address.getName().contains(this.options.getAddressNameDisplayLineBreakOn())) {
			StringJoiner multilineDisplayName = new StringJoiner("\n");
			for (String namePart : address.getName().split(this.options.getAddressNameDisplayLineBreakOn())) {
